...
namespace vq3 {
  ...
  namespace utils {
    ...
    class Vertices {
    public:
      using ref_vertex_type = REF_VERTEX;
      using vertex_type     = typename REF_VERTEX::element_type;
      using index_type      = typename std::vector<REF_VERTEX>::size_type;
    private:
      std::vector<REF_VERTEX> idx2vertex;
      std::map<vertex_type*, index_type> vertex2idx;

      friend std::ostream& operator<<(std::ostream& os, Vertices<REF_VERTEX>& v) {
    ...
    public:
      ...
      /**
       * Updates the vertices (typically after a topology change).
       */
      template<typename GRAPH>
      void update_topology(GRAPH& g) {
	clear();
	fill(g);
      }
       ...
       */
      const REF_VERTEX& operator()(index_type idx) const {return idx2vertex[idx];}
      
      /**
       * @return the index of the vertex (reference). Complexiy is logarithmic.
       */
      const index_type operator()(const REF_VERTEX& ref_v) const {
	auto pt = ref_v.get();
    ...
    template<typename GRAPH>
    auto vertices(const GRAPH&) {
      return Vertices<typename GRAPH::ref_vertex>();
    }
