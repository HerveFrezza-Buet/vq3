...
int main(int argc, char* argv[]) {
  ...
  while(keycode != 27) {
	...
	evolution.histo.gaussian(image, frame, mm, ss, aa, 50, cv::Scalar(255,0,0), 1, false);
	...
	if(cb.ref_v) {
	  ...
	  if(vertex.vq3_online_mean_std) {
	    ...
	    evolution.histo.gaussian(image, frame, m, s, aa, 50, cv::Scalar(0,0,0),   5, false);
	    evolution.histo.gaussian(image, frame, m, s, aa, 50, vertex.vq3_color,    3, false);
	  }
	}
      }
